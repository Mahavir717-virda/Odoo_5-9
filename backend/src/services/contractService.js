 ...
 */
export const checkContractOverlap = async (employee_id, start_date, end_date, excludeContractId = null) => {
  let query = `
...
export const getEmployeeContracts = async (employeeId) => {
  const empCheck = await pool.query("SELECT id, user_id FROM employees WHERE id = $1", [employeeId]);
...
export const createContract = async (data) => {
  ...
  const empRes = await pool.query("SELECT id, department, job_position FROM employees WHERE id = $1", [employee_id]);
  if (empRes.rows.length === 0) {
    const err = new Error("Employee not found");
    err.statusCode = 400;
    throw err;
  ...
  const structRes = await pool.query("SELECT id FROM salary_structures WHERE id = $1", [structure_id]);
  ...
  if (!VALID_STATUSES.includes(status)) {
    const err = new Error(`Invalid contract status. Allowed: ${VALID_STATUSES.join(", ")}`);
    err.statusCode = 400;
    throw err;
  }
  ...
  // Check date overlap
  const isOverlapping = await checkContractOverlap(employee_id, start_date, end_date);
  if (isOverlapping) {
    const err = new Error("Contract period overlaps with an existing contract for this employee");
    err.statusCode = 409;
    throw err;
  }
...
export const updateContract = async (id, data) => {
  ...
  const existingRes = await pool.query("SELECT * FROM contracts WHERE id = $1", [id]);
  ...
  if (start_date !== undefined || end_date !== undefined) {
    const isOverlapping = await checkContractOverlap(existing.employee_id, newStartDate, newEndDate, id);
    if (isOverlapping) {
      const err = new Error("Contract period overlaps with an existing contract for this employee");
      err.statusCode = 409;
      throw err;
  ...
  const updatedStructureId = structure_id !== undefined ? structure_id : existing.structure_id;
  const updatedDepartment = department !== undefined ? department : existing.department;
  const updatedJobPosition = job_position !== undefined ? job_position : existing.job_position;
...
export const activateContract = async (id) => {
  ...
  const isOverlapping = await checkContractOverlap(contract.employee_id, contract.start_date, contract.end_date, id);
  if (isOverlapping) {
    const err = new Error("Cannot activate contract because its period overlaps with another contract");
    err.statusCode = 409;
    throw err;
  }
 ...
 */
export const getContractForPeriod = async (employeeId, periodStart, periodEnd) => {
  const query = `
    SELECT
      c.*,
      ss.name AS structure_name
